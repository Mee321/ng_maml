...
acc = []
for _ in range(100):
    g = task_generator(num_classes, num_samples, n_dim)
    a_dict = train_single_task_newton(model, task_lr, criterion, g, 1)
    p = utils.flatten(a_dict.values())
    utils.set_flat_params_to(model, p)
    x, y = g.get_batch()
    logit = model(x)
    _, pred = torch.max(logit, 1)
    acc.append( (pred == y).sum().float() / num_classes)

print('%.2f' % (np.array(acc).mean()))
